...
// Mock patient context — replace with real patient props later
...
function calcFatPercent(kg: number, formula: string): number {
    ...
    const sex = PATIENT_FEMALE ? 0 : 1; // 0=female 1=male
    switch (formula) {
...
function InputCell({ value, onChange, unit, wide = false }: InputCellProps) {
    return (
        <div className="relative inline-flex items-center">
            <input
                ...
                className={`${wide ? "w-36" : "w-28"} h-9 border border-gray-200 rounded-lg px-3 text-sm font-semibold text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#1DBF73]/25 focus:border-[#1DBF73] transition-colors`}
...
function Badge({ label, bg, color }: BadgeProps) {
    return (
        <span className={`inline-flex text-[11px] font-bold px-2 py-0.5 rounded-full w-fit ${bg} ${color}`}>
            {label}
        </span>
    );
}

export function PatientValuesSection() {
    const [weightActual,   setWeightActual]   = useState("72.5");
    const [weightObj,      setWeightObj]      = useState("60.0");
    const [fatFormula,     setFatFormula]     = useState("peterson");
    const [fatObj,         setFatObj]         = useState("20.0");
                    ...
                    <thead>
                        <tr className="bg-gray-50/80 border-b border-gray-100">
                            {["Indicador", "Fórmula", "Actual", "Objetivo", "Valor de referencia"].map(h => (
                                <th key={h} className="px-4 py-3 text-left text-[11px] font-bold text-gray-400 uppercase tracking-wider first:pl-6 last:pr-6">
                                    {h}
                                </th>
                            ))}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 [&>tr:last-child>td]:pb-6">

                        {/* ── Peso ── */}
                        <tr className="hover:bg-gray-50/40 transition-colors">
                            <td className="pl-6 pr-4 py-5">
                                <div className="flex items-center gap-2.5">
                                    <div className="h-7 w-7 rounded-lg bg-blue-50 text-blue-400 flex items-center justify-center shrink-0">
                                        <Scale className="w-3.5 h-3.5" />
                                    </div>
                                    <span className="text-sm font-semibold text-gray-800">Peso</span>
                                </div>
                            </td>
                            <td className="px-4 py-5 text-sm text-gray-400">—</td>
                            <td className="px-4 py-5">
                                <InputCell value={weightActual} onChange={setWeightActual} unit="kg" />
                            </td>
                            <td className="px-4 py-5">
                                <InputCell value={weightObj} onChange={setWeightObj} unit="kg" />
                            </td>
                            <td className="pl-4 pr-6 py-5">
                                <div className="flex flex-col gap-1.5">
                                    <span className="text-sm font-semibold text-gray-800">{weightObj || "—"} kg</span>
                        ...
                        <tr className="hover:bg-gray-50/40 transition-colors">
                            <td className="px-6 py-5">
                                <div className="flex items-center gap-2.5">
                                    <div className="h-7 w-7 rounded-lg bg-cyan-50 text-cyan-500 flex items-center justify-center shrink-0">
                                        <Percent className="w-3.5 h-3.5" />
                                    </div>
                                    <span className="text-sm font-semibold text-gray-800 leading-snug">Porcentaje de<br />masa grasa</span>
                                </div>
                            </td>
                            <td className="px-4 py-5">
                                <select
                                    value={fatFormula}
                                    onChange={e => setFatFormula(e.target.value)}
                                    className="h-9 border border-gray-200 rounded-lg px-3 pr-8 text-xs font-medium text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-[#1DBF73]/25 focus:border-[#1DBF73] transition-colors cursor-pointer"
                                >
                                    {FAT_FORMULAS.map(f => (
                            ...
                            <td className="px-4 py-5">
                                <span className="text-sm font-semibold text-gray-800">{fatActual} %</span>
                            </td>
                            <td className="px-4 py-5">
                                <InputCell value={fatObj} onChange={setFatObj} unit="%" />
                            </td>
                            <td className="pl-4 pr-6 py-5">
                                <span className="text-sm font-semibold text-gray-800">{FAT_REF}</span>
                            </td>
                        </tr>
                        ...
                        <tr className="hover:bg-gray-50/40 transition-colors">
                            <td className="pl-6 pr-4 py-5">
                                <div className="flex items-center gap-2.5">
                                    <div className="h-7 w-7 rounded-lg bg-orange-50 text-orange-400 flex items-center justify-center shrink-0">
                                        <BarChart2 className="w-3.5 h-3.5" />
                                    </div>
                                    <span className="text-sm font-semibold text-gray-800 leading-snug">Índice de masa<br />corporal</span>
                                </div>
                            </td>
                            <td className="px-4 py-5 text-sm text-gray-400">—</td>
                            <td className="px-4 py-5">
                                <div className="flex flex-col gap-1.5">
                                    <span className="text-sm font-semibold text-gray-800">{bmiActual} kg/m²</span>
                                    <Badge {...bmiActualC} />
                                </div>
                            </td>
                            <td className="px-4 py-5">
                                <div className="flex flex-col gap-1.5">
                                    <InputCell
                                        value={bmiObj !== "" ? bmiObj : bmiObjNum.toString()}
                                        onChange={setBmiObj}
                                        unit="kg/m²"
                                        wide
                                    />
                            ...
                            <td className="pl-4 pr-6 py-5">
                                <div className="flex flex-col gap-1.5">
                                    <span className="text-sm font-semibold text-gray-800">{BMI_REF.text}</span>
